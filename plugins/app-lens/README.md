...
1. Inventories a user-provided APK without attempting to discover API endpoints, tokens, credentials, or backend implementation.
...
The included `scripts/static_inventory.py` accepts a local `.apk` file and writes a conservative inventory. It never decompiles code, retrieves an app from a device, or sends an APK anywhere. The workflow does not install dependencies automatically.
...
The scripts intentionally do not install software, connect to a real device, retrieve apps from a device, upload files, log in, or perform external actions.
