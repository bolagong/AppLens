...
Decompile an Android APK file for the restricted AppLens adapter.
...
case "$ext_lower" in
  apk) ;;
  *)
    echo "Error: Unsupported file type '.$ext'. Expected .apk" >&2
    exit 1
    ;;
esac
...
case "$ENGINE" in
  jadx) ;;
  *)
    echo "Error: Unsupported engine '$ENGINE'. Use jadx." >&2
    exit 1
    ;;
esac
